<template>
  <div class="create-series-container">
    <div class="page-header">
      <h1>{{ $t('admin.series.page.edit.title') }}</h1>
      <p>{{ $t('admin.series.page.edit.description') }}</p>
    </div>
...
import { getSeriesInfo, updateSeries } from '@/api/services/series'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (valid) {
      ...
      const res = await updateSeries(route.params.id as string, reqData)
      if (res.code === 1) {
        ...
        ElMessage.success(t('admin.series.page.edit.success'))
        router.push('/admin/series')
      } else {
...
const loadSeriesInfo = async () => {
  const res = await getSeriesInfo(route.params.id as string)
  if (res.code === 1) {
    seriesForm.name = res.data.name
    seriesForm.slug = res.data.slug
    seriesForm.description = res.data.series_desc
    seriesForm.coverImage = res.data.image
    seriesForm.sortOrder = res.data.articles_order === 'asc' ? 'oldest' : 'newest'
  }
}
