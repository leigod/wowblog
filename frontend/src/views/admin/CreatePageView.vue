<template>
  <div class="create-page-container">
    <div class="page-header">
      <h1>{{ $t('admin.pages.page.create.title') }}</h1>
      <p>{{ $t('admin.pages.page.create.description') }}</p>
    </div>
    ...
    >
      ...
      <!-- Page Content Editor -->
      <el-form-item
        :label="$t('admin.pages.page.form.content')"
        prop="content"
        label-position="top"
      >
...
import { ArrowLeft, Upload, Delete, ZoomIn } from '@element-plus/icons-vue'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (valid) {
      ...
      const res = await createPage(reqData)
      console.log('API response:', res)
      if (res.code === 1) {
        ElMessage.success(t('admin.pages.page.create.success'))
        ...
        ElMessage.error(t('admin.pages.page.create.error'))
      }
    }
  } catch (error) {
...
const loadContent = () => {
  ...
  if (route.query.content) {
    ...
    // 如果路由参数中没有，则尝试从localStorage获取，避免获取到之前数据，首次加载时不获取
    if (!firstLoad.value) {
