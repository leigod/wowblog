      ...
      <el-main>
        <el-row :gutter="30" justify="space-between">
          <el-col :xs="24" :sm="12" :md="12">
            <el-tag type="primary">{{ $t('series.info.tag') }}</el-tag>
            <h1>{{ seriesDetail.name }}</h1>
            <div class="wy-series-desc">{{ seriesDetail.series_desc }}</div>
          </el-col>
          <el-col :xs="24" :sm="12" :md="12" class="wy-page-cover-wrapper">
            <img
              v-if="seriesDetail.image"
        ...
        <el-row
          ><el-divider>{{ $t('series.info.title') }}</el-divider>
        </el-row>
        <el-row>
          <el-col :xs="24" class="wy-card-list">
            <el-card
              v-for="item in seriesArticlesList"
              :key="item.id"
              shadow="hover"
        ...
        <div
          ...
          v-if="hasMore && seriesArticlesList.length > 0"
        ...
        <el-empty v-if="seriesArticlesList.length === 0 && !loading" description="暂无文章" />
      </el-main>
    </el-container>
  </div>
</template>
...
import { getSeriesDetailApi, getSeriesArticlesList } from '@/api/services/blog'
...
const fetchArticles = async (page: number, size: number, append: boolean = false) => {
  ...
  try {
    // 调用API获取系列下的文章列表，传递分页参数
    const articlesRes = await getSeriesArticlesList(seriesSlug, page, size)
    ...
    if (articlesRes.code === 1) {
      ...
      if (append) {
        seriesArticlesList.value = [...seriesArticlesList.value, ...newArticles]
      } else {
        seriesArticlesList.value = newArticles
      }
      ...
      // 判断是否还有更多数据
      if (seriesArticlesList.value.length >= total.value || newArticles.length < size) {
        hasMore.value = false
      } else {
        hasMore.value = true
      }
...
onMounted(async () => {
  try {
    ...
    // 调用API获取系列详情
    const res = await getSeriesDetailApi(seriesSlug)
...
// 动态设置浏览器标题
const setPageSEO = async () => {
  // 动态设置浏览器标题和meta标签
  if (seriesDetail.value?.name) {
    const title = seriesDetail.value.name
    const description = seriesDetail.value.series_desc
    const imageUrl = seriesDetail.value.image || ''
