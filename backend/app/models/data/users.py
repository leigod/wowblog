...
class User(Base):
    ...
    login_ip = Column(String(45), default=None)
    login_time = Column(Integer, default=None)
    join_ip = Column(String(45), default=None)
    createtime = Column(Integer, default=int(time.time()))
    updatetime = Column(Integer, nullable=True, comment='资料更新时间')
    notification_check_at = Column(Integer, nullable=True, comment='通知检查时间')
    # 扩展字段
